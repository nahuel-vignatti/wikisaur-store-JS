...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const cargarBase = async () => {
    ...
    btnFiltrar.onclick = function () {
      if (document.querySelector("#juguetesFiltro").checked) {
        gestor.filtrarJuguetes();
      } else {
        if (document.querySelector("#remerasFiltro").checked) {
          gestor.filtrarRemeras();
        } else {
          if (document.querySelector("#puzzlesFiltro").checked) {
            gestor.filtrarPuzzles();
          }
        }
      }
    };
