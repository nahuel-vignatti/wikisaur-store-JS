class GestorProductos {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          Toastify({
            ...
            position: "right",
            className: "tostada",
            style: { background: "rgba(255, 0, 0, 0.637)" },
          }).showToast();
        } else {
          Toastify({
            ...
            position: "right",
            className: "tostada",
            style: { background: "rgba(0, 128, 0, 0.616)" },
          }).showToast();
        }
      });
    }
  ...
  cargaDom(coleccion) {
    ...
    coleccion.forEach((element) => {
      ...
      contenedor.innerHTML = `<h3 class="fs-1 fw-bold">${nombre}</h3> 
                              <img src="../multimedia/img/${urlImg}" alt="" class="imgProducto" >                          
                              <p class="fs-2">Precio: $${precio}</p> 
                              <p class="fs-2">Tipo: ${tipo}</p>`;
      ...
      botonAgregar.onclick = function () {
        ...
        Toastify({
          ...
          position: "right",
          className: "tostada",
          style: { background: "rgba(168, 168, 168, 0.651)"},
          }).showToast();
...
//=====================================================================//
  ...
  filtrarJuguetes(){
    ...
    coleccion_mostrada = coleccion_mostrada.filter(el => el.tipo == "Juguete");
    this.cargaDom(coleccion_mostrada);
  }
  filtrarRemeras(){
    coleccion_mostrada = coleccion_productos;
    coleccion_mostrada = coleccion_mostrada.filter(el => el.tipo == "Remera");
    this.cargaDom(coleccion_mostrada);
  }
  filtrarPuzzles(){
    coleccion_mostrada = coleccion_productos;
    coleccion_mostrada = coleccion_mostrada.filter(el => el.tipo == "Puzzle");
    this.cargaDom(coleccion_mostrada);
  }
